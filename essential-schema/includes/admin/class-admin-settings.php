...
class ES_Config_Settings {
    ...
    public function sanitize_general($input) {
        $sanitized = [];
        $sanitized['enable_article_schema'] = !empty($input['enable_article_schema']);
        return $sanitized;
    }
    ...
    public function sanitize_shipping($input){
        ...
        if (isset($input['shipping_items']) && is_array($input['shipping_items'])) {
            foreach ($input['shipping_items'] as $item) {
                $sanitized_item = [];
                $sanitized_item['rate'] = isset($item['rate']) ? (float) $item['rate'] : 0.0;
                $sanitized_item['currency'] = isset($item['currency']) ? sanitize_text_field(strtoupper($item['currency'])) : 'USD';
                $sanitized_item['description'] = sanitize_text_field($item['description'] ?? '');
                $sanitized_item['countries'] = sanitize_textarea_field($item['countries'] ?? '');
                $sanitized_item['handling_min'] = absint($item['handling_min'] ?? 0);
                $sanitized_item['handling_max'] = absint($item['handling_max'] ?? 0);
                $sanitized_item['transit_min'] = absint($item['transit_min'] ?? 0);
                $sanitized_item['transit_max'] = absint($item['transit_max'] ?? 0);
                $out['shipping_items'][] = $sanitized_item;
            }
        }
        return $out;
    }
        ...
        add_settings_field(
            ...
            function() use ($opt, $key, $desc) {
                ...
                wp_dropdown_pages([
                    'name' => $opt . '[' . $key . ']',
                    'selected' => $value,
                    'show_option_none' => __('None', 'essential-schema'),
                    'option_none_value' => 0,
                ]);
                        ...
                        $('.es-repeater').on('click', '.es-add-item', function() {
                            ...
                            if ($lastItem.length === 0) {
                                $newItem = $('<div class="es-repeater-item">' +
                                    ...
                                    '<input type="text" name="' + '<?php echo esc_attr($opt . '[' . $key . '][0][question]'); ?>" value="" class="regular-text"></p>' +
                        ...
                        <div class="es-returns-repeater-item">
                            ...
                            <select name="<?php echo esc_attr($opt . '[' . $key . '][' . $i . '][fees]'); ?>">
                                ...
                                foreach ($fees_options as $fee) {
                                    ...
                                    echo '<option value="' . esc_attr($fee) . '" ' . $selected . '>' . esc_html($fee) . '</option>';
                                }
                                ?>
                            </select></p>
                            ...
                            <select name="<?php echo esc_attr($opt . '[' . $key . '][' . $i . '][refund_type]'); ?>">
                                ...
                                foreach ($refund_options as $refund) {
                                    ...
                                    echo '<option value="' . esc_attr($refund) . '" ' . $selected . '>' . esc_html($refund) . '</option>';
                                }
                                ?>
                            </select></p>
                            ...
                            foreach ($methods as $method) {
                                ...
                                echo '<label><input type="checkbox" name="' . esc_attr($opt . '[' . $key . '][' . $i . '][return_method][]') . '" value="' . esc_attr($method) . '" ' . $checked . '> ' . esc_html($method) . '</label><br>';
                            }
                            ?>
    ...
    private function add_shipping_repeater_field($opt, $key, $label) {
        add_settings_field(
            ...
            function() use ($opt, $key) {
                ...
                $items = $options[$key] ?? [];
                ?>
                <div class="es-shipping-repeater">
                    <?php foreach ($items as $i => $item): ?>
