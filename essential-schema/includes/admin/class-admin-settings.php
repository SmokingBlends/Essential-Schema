...
class ES_Settings_Page {
    ...
    public function enqueue_scripts($hook) {
        ...
        wp_enqueue_script('es-admin-settings', ES_PLUGIN_URL . 'assets/js/admin-settings.js', ['jquery'], '1.0', true);
        wp_add_inline_script('es-admin-settings', '
            jQuery(document).ready(function($) {
                function toggleReturnFields() {
                    var category = $("#es_return_policy_category").val();
                    if (category === "MerchantReturnFiniteReturnWindow" || category === "MerchantReturnUnlimitedWindow") {
                        $(".es-return-details-wrap").show();
                        if (category === "MerchantReturnFiniteReturnWindow") {
                            $(".es-return-days-wrap").show();
                        } else {
                            $(".es-return-days-wrap").hide();
                        }
                    } else {
                        $(".es-return-details-wrap").hide();
                        $(".es-return-days-wrap").hide();
                    }
                }
                toggleReturnFields();
                $("#es_return_policy_category").on("change", toggleReturnFields);
            });
        ');
    }
}
...
class ES_Config_Settings {
    ...
    public function register_fields(){
        ...
        $this->add_select_field('es_org', 'org_type', esc_html__('Organization Type','essential-schema'), ['Organization', 'LocalBusiness', 'OnlineStore'], esc_html__('Choose schema type.','essential-schema'));
        ...
        $this->add_text_field('es_org', 'price_range', esc_html__('Price Range','essential-schema'), esc_html__('e.g., $$ (for LocalBusiness only)','essential-schema'));
        ...
        $this->add_select_field('es_domestic_returns', 'return_policy_category', esc_html__('Return Policy Category','essential-schema'), ['MerchantReturnFiniteReturnWindow', 'MerchantReturnUnlimitedWindow', 'MerchantReturnNotPermitted'], '', 'es_return_policy_category');
        $this->add_number_field('es_domestic_returns', 'days', esc_html__('Domestic Return Days','essential-schema'), '', '1', 'es-return-days-wrap');
        $this->add_select_field('es_domestic_returns', 'fees', esc_html__('Domestic Return Fees','essential-schema'), ['FreeReturn', 'ReturnFeesCustomerResponsibility'], '', '', 'es-return-details-wrap');
        $this->add_select_field('es_domestic_returns', 'refund_type', esc_html__('Refund Type','essential-schema'), ['FullRefund', 'StoreCreditRefund', 'MoneyBackOrReplacement'], '', '', 'es-return-details-wrap');
        $this->add_checkbox_field('es_domestic_returns', 'return_method', esc_html__('Return Methods','essential-schema'), ['ReturnByMail', 'ReturnInStore', 'ReturnAtKiosk'], esc_html__('Select all that apply.','essential-schema'), 'es-return-details-wrap');
        ...
        add_settings_section(
            'es_product',
            esc_html__('Product Settings','essential-schema'),
            '__return_false',
            'es_schema_settings'
        );
        $this->add_select_field('es_product', 'item_condition', esc_html__('Item Condition','essential-schema'), ['NewCondition', 'UsedCondition', 'RefurbishedCondition', 'DamagedCondition'], esc_html__('Default condition for products.','essential-schema'));
    }
    ...
    public function sanitize_org($input) {
        $sanitized = [];
        $sanitized['org_type'] = isset($input['org_type']) && in_array($input['org_type'], ['Organization', 'LocalBusiness', 'OnlineStore']) ? $input['org_type'] : 'Organization';
        ...
        $sanitized['price_range'] = isset($input['price_range']) ? sanitize_text_field($input['price_range']) : '';
    ...
    public function sanitize_product($input){
        $out = [];
        $out['item_condition'] = in_array($input['item_condition'] ?? '', ['NewCondition', 'UsedCondition', 'RefurbishedCondition', 'DamagedCondition']) ? $input['item_condition'] : 'NewCondition';
        return $out;
    }
    ...
    private function add_select_field($opt, $key, $label, $options, $desc='', $id='', $wrap_class=''){
        add_settings_field(
            ...
            function() use ($opt, $key, $options, $desc, $id, $wrap_class){
                ...
                $wrap_end = $wrap_class ? '</div>' : '';
                echo wp_kses_post($wrap_start);
                echo '<select name="' . esc_attr($opt . '[' . $key . ']') . '"' . esc_html($id_attr) . '>';
