 ...
 * - Option `es_policy_pages`: faq_page_id, returns_page_id, shipping_page_id.
 * - Safe rendering and notices when a page is unset.
 */
...
class ES_Settings_Page {
    ...
    public function render_settings_page(){
        ...
        <div class="wrap">
            ...
            <?php settings_errors('es_policy_pages'); ?>
...
class ES_Policy_Page_Settings {
    ...
    /** Sanitize and warn when unset. */
    public function sanitize($input){
        $out    = [];
        $keys   = ['faq_page_id','returns_page_id','shipping_page_id'];
        $labels = [
            'faq_page_id'      => esc_html__('FAQ Page','essential-schema'),
            'returns_page_id'  => esc_html__('Returns Policy Page','essential-schema'),
            'shipping_page_id' => esc_html__('Shipping Policy Page','essential-schema'),
        ];
        ...
        foreach ($keys as $k){
            ...
            if ( $out[$k] === 0 ) {
                $msg = sprintf(
                    // translators: %s is the field label, e.g., "FAQ Page".
                    esc_html__('%s is not selected. Related schema output will be disabled.', 'essential-schema'),
                    $labels[$k]
                );

                add_settings_error(
                    self::$option_key,
                    self::$option_key . '_' . $k . '_missing',
                    $msg,
                    'error'
                );
            }
