...
function add_missing_schema_to_product($markup, $product) {
    ...
    $shipping_policy_url = $policy_pages['shipping_page_id'] ? get_permalink($policy_pages['shipping_page_id']) : $shipping_opts['policy_url'] ?? home_url('/shipping/');
    $currency = $shipping_opts['currency'] ?? get_woocommerce_currency();
    $domestic = array_filter(array_map('trim', explode("\n", $domestic_returns['countries'] ?? 'US')));
    ...
    $return_policy = [
        ...
        'applicableCountry' => $domestic,
        'returnPolicyCategory' => 'https://schema.org/MerchantReturnFiniteReturnWindow',
        'merchantReturnDays' => (int) ($domestic_returns['days'] ?? 30),
        'returnMethod' => 'https://schema.org/ReturnByMail',
        'returnFees' => 'https://schema.org/' . ($domestic_returns['fees'] ?? 'FreeReturn'),
        'refundType' => 'https://schema.org/' . ($domestic_returns['refund_type'] ?? 'FullRefund'),
        'description' => $domestic_returns['description'] ?? 'Free returns within 30 days of delivery. Items must be unused and in original packaging.',
        'url' => $returns_url,
    ];
