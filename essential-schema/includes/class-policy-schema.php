...
class ES_Policy_Schema {
    ...
    public function get_return_policy_ids(): array {
        ...
        $ids = [];
        foreach ($return_policies as $index => $policy_item) {
            $ids[] = get_permalink($returns_page_id) . '#ReturnPolicy' . ($index + 1);
        }
        return $ids;
    }

    public function get_shipping_policy_ids(): array {
        $policy_pages = get_option('es_policy_pages', []);
        ...
        $ids = [];
        foreach ($shipping_items as $index => $item) {
            $ids[] = get_permalink($shipping_page_id) . '#ShippingPolicy' . ($index + 1);
        }
        return $ids;
    }
    ...
    public function output_returns_jsonld(): void {
        ...
        foreach ($return_policies as $index => $policy_item) {
            ...
            $policy['@id'] = get_permalink($returns_page_id) . '#ReturnPolicy' . ($index + 1);
            ...
            if (!empty($countries)) {
                $policy['returnPolicyCountry'] = (count($countries) > 1) ? $countries : $countries[0];
            }
    ...
    public function output_shipping_jsonld(): void {
        ...
        foreach ($shipping_items as $index => $item) {
            ...
            $policy['@id'] = get_permalink($shipping_page_id) . '#ShippingPolicy' . ($index + 1);
