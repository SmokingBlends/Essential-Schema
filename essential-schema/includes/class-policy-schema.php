...
class ES_Policy_Schema {
    ...
    public function output_shipping_jsonld(): void {
        ...
        if (isset($shipping_opts['rate']) && $shipping_opts['rate'] > 0) {
            $policy['shippingRate'] = [
                '@type' => 'MonetaryAmount',
                'value' => (float) $shipping_opts['rate'],
                'currency' => $shipping_opts['currency'] ?? 'USD',
            ];
        }

        if (!empty($shipping_opts['description'])) {
            $policy['description'] = $shipping_opts['description'];
        }
        ...
        if (isset($shipping_opts['handling_min']) && $shipping_opts['handling_min'] > 0 || isset($shipping_opts['handling_max']) && $shipping_opts['handling_max'] > 0 || isset($shipping_opts['transit_min']) && $shipping_opts['transit_min'] > 0 || isset($shipping_opts['transit_max']) && $shipping_opts['transit_max'] > 0) {
            $policy['deliveryTime'] = [
                '@type' => 'ShippingDeliveryTime',
                'handlingTime' => [
                    '@type' => 'QuantitativeValue',
                    'minValue' => (int) ($shipping_opts['handling_min'] ?? 0),
                    'maxValue' => (int) ($shipping_opts['handling_max'] ?? 0),
                    'unitCode' => 'DAY',
                ],
                'transitTime' => [
                    '@type' => 'QuantitativeValue',
                    'minValue' => (int) ($shipping_opts['transit_min'] ?? 0),
                    'maxValue' => (int) ($shipping_opts['transit_max'] ?? 0),
                    'unitCode' => 'DAY',
                ],
            ];
        }
